...
export const exportProjectsToExcel = (projects, employees = []) => {
  const wb = XLSX.utils.book_new();

  const projData = projects.map(p => {
    const pm = Array.isArray(employees) ? employees.find(e => e.id === p.projectManager) : null;
    return {
      'Project No': p.projectNo || p.id,
      'Project ID': p.id,
      'Project Name': p.name,
      'Client': p.clientName,
      ...
      'Demobilization': p.demobilizationDate || '',
      'Readiness %': p.readiness || 0,
      'Risk Level': p.riskLevel || 'Medium',
      'Budget (THB)': p.budget || '',
      'Actual Cost (THB)': p.actualCost || '',
      'Variance (THB)': (p.budget || 0) - (p.actualCost || 0),
    };
  });
