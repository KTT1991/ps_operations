...
function StockStatus({ item }) {
  if (item.quantity <= 0) return <span className="bg-rose-50 text-rose-700 border border-rose-200 px-2 py-0.5 rounded-md text-xs font-semibold inline-flex items-center gap-1"><AlertTriangle className="w-3 h-3" />Out of Stock</span>;
  if (item.quantity <= item.safetyStock) return <span className="bg-rose-50 text-rose-700 border border-rose-200 px-2 py-0.5 rounded-md text-xs font-semibold inline-flex items-center gap-1"><AlertTriangle className="w-3 h-3" />Critical Low</span>;
  if (item.quantity <= item.reorderLevel) return <span className="bg-amber-50 text-amber-800 border border-amber-200 px-2 py-0.5 rounded-md text-xs font-semibold inline-flex items-center gap-1"><TrendingDown className="w-3 h-3" />Reorder Now</span>;
  return <span className="bg-emerald-50 text-emerald-700 border border-emerald-200 px-2 py-0.5 rounded-md text-xs font-semibold inline-flex items-center gap-1"><Package className="w-3 h-3" />In Stock</span>;
}
...
function InventoryModal({ item, onClose, onSave }) {
  ...
  const fields = [
    ...
    [['unitCost', 'Unit Cost (THB)', 'number'], ['leadTimeDays', 'Lead Time (days)', 'number']],
  ...
  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center p-4">
      ...
      <div className="relative bg-white border-2 border-slate-300 rounded-xl w-full max-w-xl max-h-[90vh] overflow-y-auto animate-fade-in shadow-xl">
        <div className="p-5 border-b border-slate-200 flex items-center justify-between sticky top-0 bg-white">
          <h2 className="font-semibold text-slate-900">{item ? 'Edit Inventory Item' : 'Add Inventory Item'}</h2>
          <button onClick={onClose} className="btn-ghost p-1 text-slate-600 hover:text-slate-900">✕</button>
        </div>
        ...
        <div className="p-5 border-t border-slate-200 flex gap-3 justify-end sticky bottom-0 bg-white">
...
export default function InventoryPage() {
  ...
  const exportToExcel = () => {
    const data = filtered.map(i => ({
      ...
      'Unit Cost (THB)': i.unitCost, 'Total Value (THB)': i.quantity * i.unitCost,
  ...
  return (
    <div className="space-y-5 animate-fade-in">
      ...
      <div className="grid grid-cols-2 sm:grid-cols-4 gap-3">
        ...
        <div className="kpi-card">
          <div className="text-2xl font-bold text-green-400">{(totalValue / 1000).toFixed(0)}K THB</div>
          ...
          {categories.map(c => (
            <button key={c} onClick={() => setCatFilter(c)}
              className={clsx('px-3 py-1.5 rounded-lg text-xs font-semibold border transition-all',
                catFilter === c ? 'bg-orange-50 text-orange-700 border-orange-300 shadow-2xs' : 'bg-white text-slate-700 border-slate-200 hover:border-orange-300')}>
              {c}
            </button>
          ))}
          <button onClick={() => setAlertFilter(!alertFilter)}
            className={clsx('px-3 py-1.5 rounded-lg text-xs font-semibold border transition-all flex items-center gap-1.5',
              alertFilter ? 'bg-amber-100 text-amber-900 border-amber-300 shadow-2xs' : 'bg-amber-50 text-amber-800 border-amber-200 hover:bg-amber-100/60')}>
            <AlertTriangle className="w-3.5 h-3.5 text-amber-600" />Alerts Only
          </button>
        </div>
      </div>
      ...
      <div className="card overflow-hidden">
              ...
              {loading ? (
                ...
                return (
                  <tr key={item.id}>
                    ...
                    <td><span className="font-sans text-xs text-slate-400">{item.partNumber}</span></td>
                    ...
                    <td><span className="text-sm text-slate-300">{item.unitCost?.toLocaleString()} THB</span></td>
                    <td><span className="text-sm text-slate-300">{(item.quantity * item.unitCost)?.toLocaleString()} THB</span></td>
        ...
        <div className="px-4 py-3 border-t border-slate-800 text-xs text-slate-500">
          Showing {filtered.length} of {items.length} items · Total value: {totalValue.toLocaleString()} THB
        </div>
      </div>
