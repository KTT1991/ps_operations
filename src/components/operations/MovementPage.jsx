...
const STATUSES = ['Available', 'In Use', 'Under Maintenance', 'Reserved', 'Damaged', 'Standby', 'Disposal'];

function MovementModal({ type, asset, projects, assets, onClose, onSave }) {
  const { user, isAdmin, isBaseManager, isStoreman, canManageMovement, userRole } = useAuth();
  const canEdit = canManageMovement ?? (isAdmin || isBaseManager || isStoreman || userRole === 'storeman');
                    ...
                    <div className="p-5 grid grid-cols-1 md:grid-cols-2 gap-x-5 gap-y-4 max-h-[80vh] overflow-y-auto">
                        <div className='md:col-span-2'>
                            ...
                            <div className='bg-[var(--t-bg3)] border rounded-md p-2 h-32 overflow-y-scroll text-xs space-y-1'>
                                {selectedAssets.map(a => <p key={a.id} className='font-sans'>{a.assetNo} - <span className='text-[var(--t-text3)]'>{a.name}</span></p>)}
                            </div>
                        </div>
                        <div className="md:col-span-2">
...
export default function MovementPage() {
  const { isAdmin, isBaseManager, isStoreman, canManageMovement, userRole } = useAuth();
  const canEdit = canManageMovement ?? (isAdmin || isBaseManager || isStoreman || userRole === 'storeman');
              ...
              <div className="flex items-center gap-2 animate-fade-in">
                  <span className='text-sm text-[var(--t-text3)]'>{selectedAssetIds.length} items selected</span>
                   {tab === 'base' ?
                    ...
                    : <button onClick={() => setModal({ type: 'bulk-in'})} className="inline-flex items-center gap-1.5 px-3 py-1.5 rounded-lg text-xs font-semibold bg-emerald-600 hover:bg-emerald-700 text-white shadow-sm transition-colors"><HardDriveDownload className="w-4 h-4" />Bulk Load In</button>
                   }
              </div>
            ) : (
              <div className="flex items-center gap-2">
                  <button onClick={exportLog} className="btn-secondary text-xs"><Download className="w-4 h-4" />Export</button>
                  <button onClick={() => setModal({ type:'in', asset: null })} className="inline-flex items-center gap-1.5 px-3 py-1.5 rounded-lg text-xs font-semibold bg-emerald-600 hover:bg-emerald-700 text-white shadow-sm transition-colors"><ArrowLeft className="w-4 h-4" />Load In</button>
      ...
      <div className="card overflow-hidden">
        <div className="border-b border-[var(--t-border)] flex items-center">
            ...
            >
                ...
                <span className="bg-[var(--t-bg3)] text-xs font-sans px-1.5 py-0.5 rounded-full">{assetsAtBase.length}</span>
            </button>
            <button 
            ...
            >
                ...
                <span className="bg-[var(--t-bg3)] text-xs font-sans px-1.5 py-0.5 rounded-full">{assetsInField.length}</span>
            </button>
        </div>
            ...
            <tbody>
              {loading ? (
                ...
                <tr><td colSpan={canEdit ? 8 : 7} className="text-center py-10 text-[var(--t-text3)]">No items found for current filters</td></tr>
                  ...
                  return (
                    <tr key={asset.id} className={clsx(selectedAssetIds.includes(asset.id) && 'bg-orange-50/70 border-l-2 border-orange-500')}>
                      ...
                      <td><div className="text-xs text-blue-700 font-semibold truncate block max-w-[150px]" title={project?.name || projectDisplay}>{projectDisplay}</div></td>
                      {canEdit && (
                        <td>
