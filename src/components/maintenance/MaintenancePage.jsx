      ...
      <div className="relative modal-bg border rounded-xl w-full max-w-3xl max-h-[92vh] overflow-y-auto animate-fade-in shadow-2xl">
        ...
        <div className="p-5 space-y-5">
          ...
          <div className="grid grid-cols-2 gap-3">
            ...
            <div>
              <label className="text-xs text-[var(--t-text3)] block mb-1">Cost (THB)</label>
  ...
  return (
    <div className="space-y-5 animate-fade-in">
      ...
      <div className="grid grid-cols-2 sm:grid-cols-4 gap-3">
        {[
          { label: 'Total Job Cards',  value: records.length,                                      color: 'text-slate-800', bg: 'bg-gradient-to-br from-slate-50/90 via-white to-slate-100/50 border-slate-200/90' },
          { label: 'In Progress',      value: records.filter(r => r.status === 'In Progress').length, color: 'text-amber-700', bg: 'bg-gradient-to-br from-amber-50/90 via-white to-amber-50/40 border-amber-200/90' },
          { label: 'Pending',          value: records.filter(r => r.status?.startsWith('Pending')).length, color: 'text-purple-700', bg: 'bg-gradient-to-br from-purple-50/90 via-white to-purple-50/40 border-purple-200/90' },
          { label: 'Cost MTD',         value: `${(totalCost / 1000).toFixed(0)}K THB`,             color: 'text-orange-700', bg: 'bg-gradient-to-br from-orange-50/90 via-white to-orange-50/40 border-orange-200/90' },
        ].map(k => (
          <div key={k.label} className={clsx('rounded-xl border p-4 shadow-xs hover:shadow-md transition-all flex flex-col justify-between', k.bg)}>
            <div className={clsx('text-2xl font-extrabold', k.color)}>{k.value}</div>
        ...
        <div className="flex items-center flex-wrap gap-2">
          {['All', ...STATUS_OPTIONS].map(s => (
            <button key={s} onClick={() => setStatusFilter(s)}
              className={clsx('px-3 py-1.5 rounded-lg text-xs border transition-all cursor-pointer font-medium',
                statusFilter === s
          ...
          {isFiltered && (
              <button onClick={handleClearFilters} className="btn-secondary text-xs flex items-center gap-1.5 cursor-pointer">
                  <X className="w-4 h-4"/>
                  Clear Filter
              </button>
          )}
          ...
          <table className="data-table">
            ...
            <tbody>
              {loading ? (
                ...
                return (
                  <Fragment key={rec.id}>
                    <tr className="cursor-pointer" onClick={() => setExpanded(isExpanded ? null : rec.id)}>
                      <td>
                        <span className="font-sans text-xs font-semibold" style={{ color: '#f97316' }}>
                          {rec.jobCardNo || '—'}
                        </span>
                      </td>
                      <td>
                        ...
                        <div className="text-xs font-sans" style={{ color: 'var(--t-text3)' }}>{asset?.assetNo || rec.assetId}</div>
                      </td>
                      <td>
                                  ...
                                  {rec.certFields.map((c, i) => (
                                    <div key={i} className="flex items-center justify-between gap-3">
                                      ...
                                      <div className="text-right">
                                        ...
                                        {c.certNo && <div className="font-sans" style={{ color: 'var(--t-text3)' }}>{c.certNo}</div>}
                                      </div>
                                    </div>
                                  ))}
                                </div>
