...
const REPORTS = [
  ...
  },
  {
    ...
    icon: Users,
    color: 'text-cyan-600',
    border: 'border-slate-300 hover:border-cyan-500',
    formats: ['excel'],
  },
  {
...
export default function ReportsPage() {
  ...
  const generateCalibrationReport = () => {
    ...
    const data = assets
      ...
      .map(({ asset, days }) => ({
        'Asset No.': asset.assetNo || asset.id,
  ...
  const generateInventoryShortageReport = () => {
    const shortages = inventory.filter(i => i.quantity <= i.reorderLevel).map(i => ({
      ...
      'Unit Cost (THB)': i.unitCost,
      'Est. Reorder Cost (THB)': i.reorderQuantity * i.unitCost,
      'Supplier': i.supplier,
      ...
      <div>
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 xl:grid-cols-3 gap-4">
          {REPORTS.map(report => {
            ...
            return (
              <div key={report.id} className={clsx('card bg-white border rounded-xl overflow-hidden shadow-sm transition-all hover:shadow', report.border)}>
                <div className="p-4">
                  <div className="flex items-start gap-3 mb-3">
                    <div className={clsx('p-2.5 rounded-lg border border-slate-200 bg-white shadow-xs', report.color)}>
                  ...
                  <div className="flex gap-2 mt-4">
                    {report.formats.includes('excel') && (
                      <button
                        ...
                        className="flex-1 flex items-center justify-center gap-1.5 px-3 py-2 bg-emerald-50 hover:bg-emerald-100/80 text-emerald-700 border border-emerald-200 rounded-lg text-xs font-semibold shadow-xs transition-all active:scale-[0.99] disabled:opacity-50"
                      >
                    ...
                    {report.formats.includes('pdf') && (
                      <button
                        ...
                        className="flex-1 flex items-center justify-center gap-1.5 px-3 py-2 bg-rose-50 hover:bg-rose-100/80 text-rose-700 border border-rose-200 rounded-lg text-xs font-semibold shadow-xs transition-all active:scale-[0.99] disabled:opacity-50"
                      >
