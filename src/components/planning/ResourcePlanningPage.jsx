...
function GapAnalysis({ project, assets, employees, onGoToProjects }) {
  const mobDate = useMemo(() => project ? parse(project.mobilizationDate || project.startDate) : null, [project]);
  const endDate = useMemo(() => project ? parse(project.endDate || project.mobilizationDate || project.startDate) : null, [project]);

  const checks = useMemo(() => {
    if (!project || !mobDate) return { analysis: [], overallReadiness: 0, totalShortfall: 0 };
...
function AssetAvailabilityTable({ assets, searchDate }) {
  ...
  const assetStatuses = ['All', 'Available', 'In Use', 'Under Maintenance', 'Damaged'];
    ...
    <div className="space-y-4">
      <div className="flex flex-wrap gap-3">
          ...
          <div>
              ...
              <select value={categoryFilter} onChange={e => { setCategoryFilter(e.target.value); setCurrentPage(1); }} className="select-field w-full sm:w-44">
                  {assetCategories.map(c => <option key={c} value={c}>{c}</option>)}
              </select>
          </div>
          <div>
              <label className="text-xs block mb-1" style={{color:T.text3}}>Status</label>
              <select value={statusFilter} onChange={e => { setStatusFilter(e.target.value); setCurrentPage(1); }} className="select-field w-full sm:w-44">
        ...
        {rows.map(a => (
            <div key={a.id} className="card p-3 space-y-2">
              <div className="flex justify-between items-start">
                <div className="font-semibold text-sm" style={{color:T.text}}>{a.name}</div>
              ...
              <div className="text-xs grid grid-cols-2 gap-2" style={{color:T.text3}}>
                <div><span className="font-medium" style={{color:T.text2}}>ID:</span> {a.id}</div>
                <div><span className="font-medium" style={{color:T.text2}}>Category:</span> {a.category}</div>
                <div><span className="font-medium" style={{color:T.text2}}>Location:</span> {a.location}</div>
                <div><span className="font-medium" style={{color:T.text2}}>Avail. Date:</span> {a.availableDate || '-'}</div>
              </div>
              <div className={clsx('flex items-center gap-1.5 text-xs font-medium pt-2 border-t', 
                a.avail.ok?'text-green-600':'text-red-500')} style={{borderColor:T.border}}>
                {a.avail.ok ? <CheckCircle className="w-4 h-4"/> : <AlertTriangle className="w-4 h-4"/>}
                <span>On {format(searchDate,'dd MMM')}: {a.avail.reason}</span>
              </div>
            </div>
        ))}
      </div>
              ...
              {rows.map(a=>(
                <tr key={a.id}>
                  <td>
                    ...
                    <div className="text-[10px] font-sans" style={{color:T.text3}}>{a.id}</div>
                  </td>
    ...
    <div className="space-y-4">
      <div className="flex flex-wrap gap-3">
          ...
          <div>
              ...
              <select value={departmentFilter} onChange={e => { setDepartmentFilter(e.target.value); setCurrentPage(1); }} className="select-field w-full sm:w-44">
                  {departments.map(d => <option key={d} value={d}>{d}</option>)}
              </select>
          </div>
          <div>
              <label className="text-xs block mb-1" style={{color:T.text3}}>Availability</label>
              <select value={availabilityFilter} onChange={e => { setAvailabilityFilter(e.target.value); setCurrentPage(1); }} className="select-field w-full sm:w-44">
      ...
      <div className="space-y-3 md:hidden">
        {rows.map(emp => (
          <div key={emp.id} className="card p-3 space-y-2 text-xs">
             <div className="flex justify-between items-start">
                <div className="font-semibold text-sm" style={{color:T.text}}>{emp.name}</div>
                 <div className={clsx('text-xs font-medium p-1 rounded', emp.avail.ok?'bg-green-500/15 text-green-600':'bg-red-500/15 text-red-500')}>
                    {emp.avail.ok ? 'Available' : 'Unavailable'}
                 </div>
             </div>
             <div className="grid grid-cols-2 gap-1.5" style={{color:T.text3}}>
                <div><span className="font-medium" style={{color:T.text2}}>Position:</span> {emp.position}</div>
                <div><span className="font-medium" style={{color:T.text2}}>Department:</span> {emp.department}</div>
             </div>
             <div className="text-[11px] pt-2 border-t" style={{borderColor:T.border}}>
                {emp.avail.reason}
             </div>
          </div>
        ))}
...
export default function ResourcePlanningPage() {
  ...
  useEffect(() => {
    const loadData = async () => {
      setLoading(true);
      const cacheKey = 'planning_data_cache';
      ...
      if (cachedData && cachedData.data && (now - cachedData.timestamp < cacheDuration)) {
        ...
        // Fetch new data
        [a, p, e] = await Promise.all([assetsService.getAll(), projectsService.getAll(), employeesService.getAll()]);
        ...
        try {
          ...
          console.warn('ResourcePlanning cache serialization skipped:', cacheErr);
        }
      }
      ...
      if (plannedOrPreparing) {
        setSelProject(plannedOrPreparing.id);
      } else if (p.length > 0) {
        ...
        <div className="flex gap-1 rounded-lg p-1 w-fit" style={{background:T.bg2,border:`1px solid ${T.border}`}}>
          {tabs.map(t=>(
            <button key={t.id} onClick={()=>setTab(t.id)}
              className={clsx('flex items-center whitespace-nowrap gap-1.5 px-3 py-1.5 rounded-md text-xs font-medium transition-all',
        ...
        <div className="grid grid-cols-1 lg:grid-cols-3 gap-5">
          ...
          <div className="lg:col-span-2 card p-3 sm:p-5">
            {selProj ? (
              <>
                <div className="mb-4">
                  <h2 className="font-semibold text-sm sm:text-base" style={{color:T.text}}>{selProj.name}</h2>
        ...
        <div className="space-y-4">
          <div className="flex items-center gap-3 flex-wrap">
            <div>
              ...
              <input type="date" value={searchDate} onChange={e=>setSearchDate(e.target.value)} className="input-field w-full sm:w-44"/>
            </div>
